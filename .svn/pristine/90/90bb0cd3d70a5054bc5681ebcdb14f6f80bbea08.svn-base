...
<div class="message-body" :style="messageStyle" @drop="msgContUp($event)" @dragover="allowDrop($event)">
  <div class="msg-button-div">
    <div class="msg-button-left">
      <div v-for="item in msgTypeButton" :key="item.key" :class="`${msgType.type==item.key?'is-active':''} ${msgType.disabled?'disabled':''}`" @click="!msgType.disabled && changeMsgType(item.key)">{{item.label}}<div v-if="msgData[item.key].count > 0">{{msgData[item.key].count > maxLength ? `${maxLength}+` : msgData[item.key].count}}</div></div>
    </div>
  ...
  <div class="msg-cont-div message-info-dialog" :style="contStyle" v-show="msgType.show">
    ...
    <div class="msg-cont-cont msg-type-cont">
      ...
      <div class="msg-cont-ul">
        <ul v-if="filterMsgData.length > 0">
          <li v-for="(item, index) in filterMsgData" :key="index" :class="msgType.rowSelect == index ? 'row-select' : ''" @click="rowSelectClick(index)">
            <div v-if="[msgTypeButton[2].key, msgTypeButton[3].key].includes(msgType.type)" class="li-img">
            ...
            <div :class="msgType.type == msgTypeButton[0].key ? 'li-font-notice li-font' : 'li-font'">
              <div>
                ...
                <div v-if="msgType.type == msgTypeButton[2].key" class="li-font-task">{{item.taskName}}</div>
      ...
      <div v-if="msgType.rowSelect != null && ~msgType.rowSelect" class="msg-cont-info">
        <div class="info-flex">
          <div class="li-img" v-if="[msgTypeButton[2].key, msgTypeButton[3].key].includes(msgType.type)">
          ...
          <div :class="msgType.type == msgTypeButton[0].key ? 'li-font-notice li-font' : 'li-font'">
            <div>
              ...
              <div v-if="msgType.type == msgTypeButton[2].key" class="li-font-task">{{filterMsgData[msgType.rowSelect].taskName}}</div>
            </div>
            <div v-if="[msgTypeButton[2].key, msgTypeButton[3].key].includes(msgType.type)">
    ...
    <div class="msg-cont-footer msg-type-footer">
      <el-button v-if="[msgTypeButton[2].key, msgTypeButton[3].key].includes(msgType.type)" type="primary" @click="batchRead()">全部已读</el-button>
...
export default {
  ...
  mounted () {
    ...
    window.addEventListener('resize', () => {
      this.$nextTick(() => {
        return (() => {
          var x = this.contStyle.x
          var width = parseInt(this.contStyle.width.split('px')[0])
          var windowX = window.screenX + (window.outerWidth - window.innerWidth) / 2
          var windowWidth = window.innerWidth
          console.log(this.contStyle)
          if (parseInt(this.contStyle.left.split('px')[0]) == 0 || x <= windowX) {
            this.contStyle.left = '0px'
            this.contStyle.right = windowWidth - width + 'px'
            this.contStyle.x = windowX
          } else if (parseInt(this.contStyle.right.split('px')[0]) == 0 || x >= windowX + windowWidth - width) {
            this.contStyle.left = windowWidth - width + 'px'
            this.contStyle.right = '0px'
            this.contStyle.x = windowX + windowWidth - width
          } else {
            this.contStyle.left = x - windowX + 'px'
            this.contStyle.right = windowWidth - (x - windowX + width) + 'px'
          }
          var y = this.contStyle.y
          var height = parseInt(this.contStyle.height.split('px')[0])
          var windowY = window.screenY + window.outerHeight - window.innerHeight - 8
          var windowHeight = window.innerHeight
          if (parseInt(this.contStyle.top.split('px')[0]) <= 0 || y <= windowY) {
            this.contStyle.top = '0px'
            this.contStyle.bottom = windowHeight - height + 'px'
            this.contStyle.y = windowY
          } else if (parseInt(this.contStyle.bottom.split('px')[0]) == 0 || y + height >= windowY + windowHeight) {
            this.contStyle.top = windowHeight - height + 'px'
            this.contStyle.bottom = '0px'
            this.contStyle.y = windowY + windowHeight - height
          } else {
            this.contStyle.top = y - windowY + 'px'
            this.contStyle.bottom = windowHeight - (y - windowY + height) + 'px'
          }
        })()
      })
    })
      ...
      if (obj) {
        if (this.msgType.type == obj.key) {
          ...
          switch (obj.key) {
            ...
            var arr = data['msg_subtype'].split('.')
...
.msg-cont-div>.msg-type-footer {
  display: flex;
}
.msg-cont-div>.msg-type-footer>.el-button {
  flex-grow: 1;
}
.msg-cont-div>.msg-type-footer>.el-button:not(:last-child) {
  margin-right: 12px;
}
.msg-cont-cont {
  width: 100%;
