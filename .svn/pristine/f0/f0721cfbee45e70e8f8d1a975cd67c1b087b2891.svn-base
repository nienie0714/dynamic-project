  ...
  <div v-if="menuData.show" ref="rightMenu" class="right-menu-pop" :style="`top: ${menuData.style.y}px; left: ${menuData.style.x}px;`">
    <ul>
      ...
      @click="handleClick(item)">
        ...
        <template v-if="item.hasOwnProperty('children') && (hoverIndex == index)">
          <div class="right-menu-pop" :style="childStyle">
            <ul>
              <li v-for="(child, idx) in item.children" :key="idx" :class="child.disabled ? 'disabled' : ''" @click.stop="handleClick(child)">
                {{ child.label }}
              </li>
            </ul>
          </div>
  ...
  methods: {
    ...
    handleHover (index) {
      if (this.hoverIndex == index) {
        ...
        this.childStyle.left = width + 'px'
        this.childStyle.top = 36 * index + 'px'
      }
    },
    handleRemoveHover () {
      this.hoverIndex = null
