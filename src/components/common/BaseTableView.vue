<template>
  ...
  :header-cell-style="{'text-align': 'center'}" @row-contextmenu="rowContextmenu" @selection-change="handleSelectionChange">
    ...
    <el-table-column v-if="tableData.isOperat ? false : true" :label='"操作"' width="190" class-name="table-opt-col">
      <template slot-scope="scope">
        <div class="tool-div-button button-detail" @click="handleDetail(scope.row)"></div>
        <div v-if="permissions.update" class="tool-div-button button-edit" @click="handleEdit(scope.row)"></div>
        <div v-if="permissions.delete" class="tool-div-button button-delete" @click="handleDelete(scope.row)"></div>
