...
export default {
  ...
  created () {
    ...
    if (!this.resetUrl || this.resetUrl == '') {
      this.resetUrl = this.baseUrl + '/resetPWD'
    }
  ...
  methods: {
    ...
    saveEdit (data) {
      var obj = this.customSaveBefore(data)
      if (obj != null) {
        postData(this.editUrl, obj).then(response => {
          if (response.data.code == 0) {
            this.formData.visible = false
            this.showSuccess('编辑')
            this.customMethod()
    ...
    formatterDay (row, column) {
      if (row.hasOwnProperty('field') && row.hasOwnProperty('item')) {
        return row.item[row.field.prop] ? row.item[row.field.prop].substr(0, 10) : ''
      } else {
    ...
    formatterMin (row, column) {
      if (row.hasOwnProperty('field') && row.hasOwnProperty('item')) {
        return row.item[row.field.prop] ? row.item[row.field.prop].substr(11, 5) : ''
      } else {
