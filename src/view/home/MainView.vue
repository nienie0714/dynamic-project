...
export default {
  ...
  methods: {
    handlePage (name) {
      if (_.map(this.homeRouterData, 'moduleName').includes(name)) {
        let item = _.find(this.homeRouterData, ['moduleName', name])
        if (this[item.winName] && this[item.winName].closed) {
        } else {
          var basicName = localStorage.getItem(item.routerName)
          if (basicName) {
            name = basicName
          }
        }
        ...
        this[item.winName] = window.open(href, name)
      } else if (name == '指挥监控') {
