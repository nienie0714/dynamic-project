...
export default {
    ...
    return {
      ...
      formData: {
        ...
        formData: [
          ...
          {key: 'aircraftType', label: '机型', type: 'select', filterable: true, optionsKey: 'AircraftType', itemKey: 'aircraftIcao', itemLabel: 'aircraftIcao'},
          {
            key: 'execArr',
            label: '每周班次',
            type: 'checkbox',
          ...
          {key: 'season', label: '班期季节', type: 'tabs', tabsKey: 'scheduleSeason', options: [], change: this.changeSeason},
          {
            key: 'dateRange',
            key1: 'beginDate',
            key2: 'endDate',
            ...
            min: '',
            max: '',
            // rangeMethod: this.dateRangeReg,
            regInfo: '开始日期必须小于等于结束日期',
            dateType: 'date',
            required: 3,
            format: 'yyyy-MM-dd',
  ...
  methods: {
    changeSeason (value, callback) {
      let obj = _.find(this.formData.formData, ['key', 'dateRange'])
      let dateRange = [{
        key: obj.key1,
        value: ''
      }, {
      ...
      if (value == this.dateRange.winSpr.key) {
        obj.min = this.dateRange.winSpr.date.beginDate
        obj.max = this.dateRange.winSpr.date.endDate
      } else {
        obj.min = this.dateRange.sumAut.date.beginDate
        obj.max = this.dateRange.sumAut.date.endDate
      }
      dateRange[0].value = obj.min
      dateRange[1].value = obj.max
      callback(dateRange)
      return null
    },
    getDateRange () {
      this.$store.commit('setOption', this.dateRange.optionKey)
      let dateRange = this.$store.getters.getOption
      this.dateRange.winSpr.key = _.find(dateRange, ['value', this.dateRange.winSpr.value]).key
      this.dateRange.sumAut.key = _.find(dateRange, ['value', this.dateRange.sumAut.value]).key
      this.dateRange.winSpr.date = JSON.parse(_.find(dateRange, ['value', this.dateRange.winSpr.value]).remark)
      this.dateRange.sumAut.date = JSON.parse(_.find(dateRange, ['value', this.dateRange.sumAut.value]).remark)
    },
    ...
    // 新增
    handleAdd () {
      this.changeSeason(this.dateRange.sumAut.key, (param) => {
        let obj = _.find(this.formData.formData, ['key', 'dateRange'])
        let temp = {}
        _.forEach(param, item => {
          this.$set(temp, item.key, item.value)
        })
        this.$set(obj, 'defaultValue', temp)
      })
