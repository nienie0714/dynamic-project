  ...
  <el-container direction="vertical">
    ...
    <el-main class="page-table-view mult-title-table">
      <div class="page-table-header">
        ...
        <Tool-button-view :permissions="permissions" :selectionCount="tableData.multipleSelection.length" @handleDownload="handleDownload" @handleAdd="handleAdd" @handleDelete="handleDelete">
          <template slot="button-slot-scope">
            <div v-if="permissions.import" class="tool-div-button tool-bind" @click="handleImport"></div>
          </template>
        </Tool-button-view>
      ...
      <Table-view :permissions="permissions" :tableData="tableData" ref="basicTable" @handleDetail="handleDetail" @handleEdit="handleEdit" @handleDelete="handleDelete">
        <!-- <template slot="mult-fields-slot-scope" slot-scope="scope">
...
import {upload, download} from '../../../api/base.js'
...
export default {
    ...
    return {
      ...
      formData: {
        ...
        formData: [
          ...
          {key: 'startStation', label: '始发站', type: 'select', filterable: true, getOptions: '/basicdata/airport/queryAll', itemKey: 'airportIata', itemLabel: 'briefC'},
          {key: 'terminalStation', label: '目的站', type: 'select', filterable: true, getOptions: '/basicdata/airport/queryAll', itemKey: 'airportIata', itemLabel: 'briefC'},
          {key: 'inOutFlag', label: '进出标识', type: 'tabs', tabsKey: 'fltType', options: []},
          ...
          // {key: 'sta', label: '计划降落', type: 'time', valueFormat: 'HH:mm', pickerOpt: {format: 'HH:mm'}},
          {
            ...
            tabelFields: [
              ...
              {prop: 'std', label: '计划起飞', type: 'time', width: '110', field: 'HH:mm', valueFormat: 'HH:mm', pickerOpt: {format: 'HH:mm'}},
              {prop: 'sta', label: '计划降落', type: 'time', width: '110', field: 'HH:mm', valueFormat: 'HH:mm', pickerOpt: {format: 'HH:mm'}}
            ],
            tempData: {},
            tempArr: []
          }
      ...
      tableData: {
        ...
        multipleSelection: [],
        fields: [
          {prop: 'inOutFlag', label: '进出标识', fixed: 'left', hidden: false, optionKey: 'fltType', width: 50, overflow: true},
          {prop: 'flightNo', label: '航班号', fixed: 'left', hidden: false, optionKey: 'none', width: 100, overflow: true},
          {prop: 'attr', label: '属性', fixed: 'left', hidden: false, optionKey: 'attr', width: 50, overflow: true},
          {prop: 'taskCn', label: '航班任务', fixed: 'left', hidden: false, width: 60, overflow: true},
          ...
          {prop: 'aircraftType', label: '机型', fixed: 'left', hidden: false, width: 50},
          ...
          {prop: 'beginDate', label: '计划开始', fixed: false, hidden: false, width: 98},
          {prop: 'endDate', label: '计划结束', fixed: false, hidden: false, width: 98},
          {prop: 'season', label: '季节', fixed: 'right', hidden: false, optionKey: 'scheduleSeason', width: 70, overflow: true},
          {
            ...
            fixed: 'right',
            hidden: false,
            method: this.showMultColumnST,
            fields: [
              {prop: 'startStationCn', label: '始发站', fixed: false, hidden: false, width: 70},
              {prop: 'std', label: '起飞', fixed: false, hidden: false, formatter: this.formatterCharHHMMVal, width: 60}
            ]
          },
          {
            label: '经停站1',
            ...
            fixed: 'right',
            hidden: false,
            method: this.showMultColumn,
            fields: [
              {prop: 'stationCn', label: '经停站', fixed: false, hidden: false, width: 70},
              {prop: 'std', label: '起飞', fixed: false, hidden: false, formatter: this.formatterCharHHMMVal, width: 60},
              {prop: 'sta', label: '降落', fixed: false, hidden: false, formatter: this.formatterCharHHMMVal, width: 60}
            ]
          },
          {
            label: '经停站2',
  ...
  methods: {
    ...
    showMultColumn (obj, callback) {
      let value = ''
      if (obj.row[obj.field.prop] && obj.row[obj.field.prop][obj.field.index]) {
        if (obj.fld.hasOwnProperty('formatter')) {
          obj.fld.formatter(obj.row[obj.field.prop][obj.field.index][obj.fld.prop], param => {
            value = param
          })
        } else {
          value = obj.row[obj.field.prop][obj.field.index][obj.fld.prop]
        }
      } else {
        value = ''
      }
    ...
    showMultColumnST (obj, callback) {
      let value = ''
      if (obj.row[obj.fld.prop]) {
        if (obj.fld.hasOwnProperty('formatter')) {
          obj.fld.formatter(obj.row[obj.fld.prop], param => {
            value = param
          })
        } else {
          value = obj.row[obj.fld.prop]
        }
      } else {
        value = ''
      }
            ...
            _.forEach(this.formData.formData[i].tabelFields, (item) => {
              if (item.hasOwnProperty('propObj')) {
                _.forEach(this.formData.formData[i].tempArr, (temp) => {
                  ...
                  this.$set(item, item.propObj, obj)
                })
              } else {
                _.forEach(this.formData.formData[i].tempArr, (temp) => {
                  temp[item.prop] = temp[item.prop] ? temp[item.prop].substr(0, 2) + ':' + temp[item.prop].substr(2, 2) : ''
                })
              }
            })
            this.formData.formData[i].editIndex = null
