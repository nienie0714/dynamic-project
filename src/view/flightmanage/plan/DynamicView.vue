  ...
  <el-container direction="vertical">
    ...
    <el-main class="whole-table-view mult-title-table">
      <div class="whole-table-header">
        ...
        <Tool-button-view :permissions="permissions" :selectionCount="0" @handleDownload="handleDownload" @handleAdd="handleAdd" @handleDelete="handleDelete">
    ...
    <el-dialog  :visible.sync="statusData.visible" :close-on-click-modal="false" width="800px" append-to-body :class="statusData.title == '更改飞机' && statusData.jointFlag == 1 ? 'other-dialog menu-status-dialog change-aircraft-dialog' : 'other-dialog menu-status-dialog'" @close="closeStatusDialog">
      <div slot="title" class="dialog-header">
        <img :src="require('@img/title_deco.png')" />
        <span class="header-title">{{statusData.title}}</span>
      </div>
      <div class="dialog-body">
        <div v-if="!['批量发布延误'].includes(statusData.title)">
          <el-row :gutter="20">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="4">
              ...
              <div v-if="statusData.data.key == 'abnormalStatus' || statusData.data.key == 'permission' || statusData.data.key == 'exchange' || statusData.data.key == 'jointFlt'">进出港</div>
            </el-col>
            ...
            <el-col :span="8" v-if="statusData.data.key == 'abnormalStatus' || statusData.data.key == 'permission' || statusData.data.key == 'exchange' || statusData.data.key == 'jointFlt'">
        ...
        <template v-if="['发布开始值机', '发布值机截止', '发布开始登机', '发布催促登机'].includes(statusData.title)">
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-select v-model="statusData.data.checkinRegionArray" multiple placeholder="请选择" clearable>
            ...
            <el-col :span="8">
              <el-select v-model="statusData.data.checkinCounterArray" multiple placeholder="请选择" clearable>
        ...
        <!-- ··········· 异常 ··········· -->
        <template v-if="statusData.data.key == 'abnormalStatus' || statusData.data.type == 'permission' || statusData.data.key == 'exchange'">
          <el-row :gutter="20">
        ...
        <template v-if="statusData.data.key == 'abnormalStatus' || ['批量发布延误'].includes(statusData.title)">
          <el-row :gutter="20">
        ...
        <template v-if="['发布航班备降'].includes(statusData.title)">
          <el-row :gutter="20" v-if="statusData.data.inOutFlag == 'A'">
            <el-col :span="4"><div>备降站</div></el-col>
            <el-col :span="8">
              <el-select v-model="statusData.data.alternateStation" placeholder="请选择" clearable>
                <el-option v-for="option in statusData.options.alternateStationList" :key="option.airportIata" :label="option.briefC" :value="option.airportIata">
                </el-option>
              </el-select>
            </el-col>
          </el-row>
        ...
        <!-- ··········· 允许登机 ··········· -->
        <template v-if="statusData.data.key == 'permission'">
          <el-row :gutter="20">
            <el-col :span="4"><div>允许登机时间</div></el-col>
            <el-col :span="8">
              <el-date-picker v-model="statusData.data.gateTime" type="datetime" placeholder="请选择时间" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
            </el-col>
          </el-row>
        </template>
        <!--··········· 批量发布延误 ········-->
        <div v-if="['批量发布延误'].includes(statusData.title)">
          <el-table height="307" ref="multipleTable" :data="statusData.dlyData.tableData" stripe :highlight-current-row="true" align="center" style="width: 100%" @selection-change="handleDlySelectionChange">
            ...
            <el-table-column prop="inOutFlagCn" label="进出港"></el-table-column>
          </el-table>
        </div>
        <!--··········· 航班拼接 ········-->
        <div v-if="['航班拼接'].includes(statusData.title)">
          <el-table height="235" ref="multipleTable" :data="statusData.jointData.tableData" stripe :highlight-current-row="true" align="center" style="width: 100%" @current-change="handleJointSelectionChange">
          <el-table-column label="操作"  width="55" fixed="left">
        ...
        <!-- ··········· 更换飞机 ··········· -->
        <template v-if="statusData.data.key == 'exchange' && statusData.jointFlag == 0">
          <el-row :gutter="20">
            <el-col :span="4"><div>新机型</div></el-col>
            <el-col :span="8">
              <el-select v-model="statusData.data.newAircraftType" placeholder="请选择" clearable>
                <el-option v-for="option in statusData.options.aircraftTypeList" :key="option.aircraftIcao" :label="option.aircraftIcao" :value="option.aircraftIcao">
                </el-option>
              </el-select>
            </el-col>
            <el-col :span="4"><div></div>新机号</el-col>
            <el-col :span="8">
              <el-input type="input" v-model.trim="statusData.data.newAircraftNo"  placeholder="请输入新机号"></el-input>
            </el-col>
          </el-row>
        </template>
        <!-- ··········· 更换飞机(拼接)todo：未测，待航班动态拼接做完后再测 ··········· -->
        <template v-if="statusData.data.key == 'exchange' && statusData.jointFlag == 1">
        <hr>
          <el-row :gutter="20">
            <el-col :span="4"><div>航空公司</div></el-col>
            <el-col :span="8"><el-input type="input" v-model.trim="statusData.data.airlineCnD" disabled></el-input></el-col>
            <el-col :span="4"><div></div>航班号</el-col>
            <el-col :span="8"><el-input type="input" v-model.trim="statusData.data.flightNoD" disabled></el-input></el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="4"><div>机型</div></el-col>
            <el-col :span="8"><el-input type="input" v-model.trim="statusData.data.aircraftTypeD" disabled></el-input></el-col>
            <el-col :span="4"><div></div>机号</el-col>
            <el-col :span="8"><el-input type="input" v-model.trim="statusData.data.aircraftNoD" disabled></el-input></el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="4"><div>机位</div></el-col>
            <el-col :span="8"><el-input type="input" v-model.trim="statusData.data.standD" disabled></el-input></el-col>
            <el-col :span="4">
              <div>进出港</div>
            </el-col>
            <el-col :span="8" v-if="statusData.data.key == 'abnormalStatus' || statusData.data.key == 'permission' || statusData.data.key == 'exchange'">
              <el-radio-group v-model="statusData.data.inOutFlagD" disabled>
                <el-radio-button v-for="option in statusData.options.inOutFlagList" :key="option.key" :label="option.key"> {{ option.value }}</el-radio-button>
              </el-radio-group>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="4"><div></div>任务</el-col>
            <el-col :span="8">
              <el-input type="input" v-model.trim="statusData.data.taskCnD" disabled></el-input>
            </el-col>
            <el-col :span="4"><div>属性</div></el-col>
            <el-col :span="8">
              <el-radio-group v-model="statusData.data.attrD" disabled>
                <el-radio-button v-for="option in statusData.options.attrList" :key="option.key" :label="option.key"> {{ option.value }}</el-radio-button>
              </el-radio-group>
            </el-col>
          </el-row>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="!['批量发布延误'].includes(statusData.title) && !['航班拼接'].includes(statusData.title)" type="primary" :loading="statusData.loading" @click="handleStatusConfirm()">确 定</el-button>
        <el-button v-if="['批量发布延误'].includes(statusData.title)" type="primary" :loading="statusData.loading" @click="handleDlyBatchConfirm()">确 定</el-button>
        <el-button v-if="['航班拼接'].includes(statusData.title)" type="primary" :loading="statusData.loading" @click="handleJointConfirm()">确 定</el-button>
      </div>
    </el-dialog>
    ...
    <Warning-box-view :data="recoveryStandData" @handleConfirm="handleRecoveryStandConfirm" @handleClose="closeRecoveryStandDialog"></Warning-box-view>
...
export default {
    ...
    return {
          ...
          },
          {
            ...
            children: [
              ...
              },
              {
                ...
                data: {
                  title: '航班拼接',
                  url: '/dynamicFlight/jointFlt', // todo:航班拼接Url
                  type: 'jointFlt',
                  inOutFlag: 'A',
                  key: 'jointFlt',
                  value: 'joint'
              ...
              },
              {
                ...
                data: {
                  title: '航班拼接',
                  url: '/dynamicFlight/breakJointFlt', // todo:断开航班拼接Url
                  type: 'breakJointFlt',
                  inOutFlag: 'A',
                  key: 'breakJointFlt',
                  value: 'flt'
                ...
                children: [
                  {
                    ...
                    label: '进港',
                    disabled: false,
                    method: this.recoveryStandChange,
                    data: {
      ...
      formData: {
        ...
        formData: [
          ...
          {key: 'execDate', label: '执行日期', type: 'date', format: 'yyyy-MM-dd', valueFormat: 'yyyy-MM-dd', defaultValue: '', pickerOpt: this.pickerOptMethod},
          {key: 'attr', label: '属性', type: 'tabs', options: []},
          ...
          {key: 'alternateStation', label: '备降站', type: 'select', filterable: true, itemKey: 'airportIata', itemLabel: 'briefC', holdRule: true},
          {key: 'alternateReason', label: '备降原因', type: 'select', filterable: true, optionsKey: 'AbnormalReason', itemKey: 'reasonCode', itemLabel: 'nameC'},
          ...
          {key: 'beltArray', label: '行李转盘', type: 'select', multiple: true, saveKey: 'belt', filterable: true, getOptions: '/airportResource/terminalResource/queryAll', itemKey: 'resourceId', itemLabel: 'resourceNo', itemValue: 'resourceNo', optionsQuery: {resourceType: 'BELT'}},
          {key: 'checkinRegionArray', label: '值机区域', type: 'select', multiple: true, saveKey: 'checkinRegion', filterable: true, getOptions: '/airportResource/terminalArea/queryAll', itemKey: 'terminalAreaNo', itemLabel: 'name', optionsQuery: {}},
          {key: 'checkinCounterArray', label: '值机柜台', type: 'select', multiple: true, saveKey: 'checkinCounter', filterable: true, getOptions: '/airportResource/terminalResource/queryAll', itemKey: 'resourceId', itemLabel: 'resourceNo', itemValue: 'resourceNo', optionsQuery: {resourceType: 'COUNTER'}},
          {key: 'gateArray', label: '登机口', type: 'select', multiple: true, saveKey: 'gate', filterable: true, getOptions: '/airportResource/gate/queryAll', itemKey: 'gateNo', itemLabel: 'gateNo', optionsQuery: {}},
          {key: 'chuteArray', label: '行李滑槽', type: 'select', multiple: true, saveKey: 'chute', filterable: true, getOptions: '/airportResource/terminalResource/queryAll', itemKey: 'resourceId', itemLabel: 'resourceNo', itemValue: 'resourceNo', optionsQuery: {resourceType: 'CHUTE'}},
  ...
  mounted () {
    ...
    this.$store.commit('setOption', this.options.attr.key)
    this.options.attr.options = _.filter(this.$store.getters.getOption, (item) => {
      return item.key != 'M'
    })
    this.$set(this.formData.formData[4], 'options', this.options.attr.options)
  ...
  methods: {
    ...
    setResource (value, callback) {
      ...
      let urls = [
        '/airportResource/terminalResource/queryAll',
        '/airportResource/terminalArea/queryAll',
        '/airportResource/terminalResource/queryAll',
        '/airportResource/gate/queryAll',
        '/airportResource/terminalResource/queryAll'
      ]
      // let datas = _.fill(Array(5), {})
      let datas = [
        {resourceType: 'BELT', terminalNo: value},
        {terminalNo: value},
        {resourceType: 'COUNTER', terminalNo: value},
        {terminalNo: value},
        {resourceType: 'CHUTE', terminalNo: value}
      ]
      let that = this
      postAllData(urls, datas).then(this.axios.spread((res1, res2, res3, res4, res5) => {
        if (res1.data.code == 0) {
          this.$set(this.formData.formData[24], 'options', res1.data.data)
        } else {
          this.showError('获取行李转盘', '请重新尝试 !')
        }
        if (res2.data.code == 0) {
          this.$set(this.formData.formData[25], 'options', res2.data.data)
        } else {
          this.showError('获取值机区域', '请重新尝试 !')
        }
        if (res3.data.code == 0) {
          this.$set(this.formData.formData[26], 'options', res3.data.data)
        } else {
          this.showError('获取值机柜台', '请重新尝试 !')
        }
        if (res4.data.code == 0) {
          this.$set(this.formData.formData[27], 'options', res4.data.data)
        } else {
          this.showError('获取登机口', '请重新尝试 !')
        }
        if (res5.data.code == 0) {
          this.$set(this.formData.formData[28], 'options', res5.data.data)
        } else {
          this.showError('获取行李滑槽', '请重新尝试 !')
        }
        callback(resource)
        return null
      })).catch(() => {
        this.showError('获取次类资源信息', '请重新尝试 !')
      })
    },
        ...
        if (resa.data.code == 0) {
          ...
          for (let i = 0; i < this.formData.formData.length; i++) {
            if (this.formData.formData[i].key == 'execDate') {
              this.nowTime = JSON.parse(JSON.stringify(this.newTime))
            ...
            if (['belt', 'checkinRegion', 'checkinCounter', 'gate', 'chute'].includes(this.formData.formData[i].key)) {
              this.$delete(this.formData.formData[i].optionsQuery, 'terminalNo')
            }
          }
    ...
    handleDetail (row) {
      ...
      if (~_.findIndex(this.options.fltType.options, ['key', row['inOutFlag']])) {
        this.$set(row, this.formData.key, row[this.formData.key + row['inOutFlag']])
    ...
    handleEdit (row) {
      console.log(row)
      if (row.endFlag == 'Y') {
        this.showErrorCustom('当前航班已结束', '无法编辑')
        return
      }
      ...
      for (let i = 0; i < this.formData.formData.length; i++) {
        ...
        if (['belt', 'checkinRegion', 'checkinCounter', 'gate', 'chute'].includes(this.formData.formData[i].key)) {
          this.$set(this.formData.formData[i].optionsQuery, 'terminalNo', row.terminalNo)
        }
        if (this.formData.formData[i].key == 'execDate') {
          this.nowTime = JSON.parse(JSON.stringify('1970-01-01'))
          this.$delete(this.formData.formData[i], 'pickerOpt')
        }
      }
    },
    editConfirm (row) {
    ...
    handleDelete (row) {
      if (row.endFlag == 'Y') {
        this.showErrorCustom('当前航班已结束', '无法删除')
        return
      }
      let standFree = _.isNull(row.stand) || row.stand == ''
      let beltFree = _.isNull(row.belt) || row.belt == ''
      let gateFree = _.isNull(row.gate) || row.gate == ''
      let checkinCounterFree = _.isNull(row.checkinCounter) || row.checkinCounter == ''
      if (!standFree || !beltFree || !gateFree || !checkinCounterFree) {
        this.showErrorCustom('当前航班有资源占用，请回收资源 ！', '无法删除')
        return
      }
      ...
      if (data.hasOwnProperty('dynamicFlightId')) {
        _.forEach(items, (obj) => {
          ...
          if (obj.hasOwnProperty('reg')) {
            ...
            if (obj.index == '0-0-2') {
              let arr = obj.index.split('-')
              if (data.inOutFlag == 'D' && data.progressStatus == null) {
                this.$set(that.menuData.items[arr[0]].children[arr[1]].children[arr[2]], 'disabled', false)
              } else if (data.inOutFlag == 'A|D' && data.progressStatus == 'ARR') {
                this.$set(that.menuData.items[arr[0]].children[arr[1]].children[arr[2]], 'disabled', false)
              } else {
                this.$set(that.menuData.items[arr[0]].children[arr[1]].children[arr[2]], 'disabled', true)
              }
            }
          }
        })
    ...
    openStatusDialog (data, item) {
      ...
      if (item.type == 'publish' && item.value == 'ONR') {
        ...
        this.statusData.options.checkinRegionList = []
        // 开始值机  值机截止 开始登机
        queryAll('/airportResource/terminalArea/queryAll', {terminalNo: data.terminalNo == undefined ? null : data.terminalNo}).then(res => {
          if (res.data.code == 0) {
            this.statusData.options.checkinRegionList = res.data.data
            queryAll('/airportResource/terminalResource/queryAll', {resourceType: 'COUNTER', terminalNo: data.terminalNo == undefined ? null : data.terminalNo}).then(resa => {
              this.statusData.options.checkinCounterList = resa.data.data
              this.$set(statusData.data, 'checkinRegionArray', data.checkinRegionArray == undefined ? [] : data.checkinRegionArray)
              this.$set(statusData.data, 'checkinCounterArray', data.checkinCounterArray == undefined ? [] : data.checkinCounterArray)
              this.$set(statusData.data, 'counterFlag', true)
            })
          } else {
            this.showError('获取值机区域列表', '请重新尝试 !')
          }
        })
      ...
      if (item.key == 'abnormalStatus' && item.type == 'publish') {
        queryAll('/basicdata/abnormalreason/queryAll', {abnormalFlag: 'Y'}).then(res => {
          if (res.data.code == 0) {
            ...
            this.showError('获取异常状态列表', '请重新尝试 !')
          }
        })
      }
      if ((item.key == 'abnormalStatus' && item.type == 'publish') || item.type == 'permission' || item.type == 'exchange') {
      ...
      if (item.type == 'publish' && item.key == 'abnormalStatus' && item.value == 'DLY') {
        // 航班延误  本站计划时间 预计时间
      ...
      } else if (item.type == 'publish' && item.key == 'abnormalStatus' && item.value == 'ALT') {
        // 备降
        queryAll('/basicdata/airport/queryAll').then(res => {
          if (res.data.code == 0) {
            this.statusData.options.alternateStationList = res.data.data
            this.$set(statusData.data, 'alternateStation', data.alternateStation)
          } else {
            this.showError('获取备降站列表', '请重新尝试 !')
          }
        })
      } else {}
      ...
      if (item.type == 'permission') {
        this.$set(statusData.data, 'gateTime', null)
      }

      // 航班是否拼接
      statusData.jointFlag = statusData.data.inOutFlag.indexOf('|') == -1 ? 0 : 1
      // 换飞机
      if (item.key == 'exchange') {
        // 不是拼接航班
        if (!statusData.jointFlag) {
          queryAll('/basicdata/aircraftType/queryAll').then(res => {
            if (res.data.code == 0) {
              this.statusData.options.aircraftTypeList = res.data.data
              this.$set(statusData.data, 'newAircraftType', null)
            } else {
              this.showError('获取机型列表', '请重新尝试 !')
            }
          })
          // {key: 'aircraftNo', label: '飞机号', type: 'select', filterable: true, getOptions: '/basicdata/aircraft/queryAll', itemKey: 'aircraftNo', itemLabel: 'aircraftNo', change: this.changeArcft},
          this.$set(statusData.data, 'newAircraftNo', null)
        } else {
          _.forEach(statusData.data, (value, key) => {
            let i = value.indexOf('|')
            if (i != -1) {
              this.$set(statusData.data, key + 'D', value.slice(i + 1))
            }
          })
      ...
      // ------- 航班拼接 -------------
      if (item.key == 'jointFlt') {
        this.$set(statusData.data, 'inOutFlag', data.inOutFlag)
        this.$store.commit('setOption', 'fltType')
        this.statusData.options.inOutFlagList = _.cloneDeep(this.$store.getters.getOption)
        let arr = _.cloneDeep(this.tableData.multipleSelection)
        arr.forEach(item => {
          item.checked = false
        })
        this.statusData.jointData.tableData = arr // todo:选择某条进/出航班,系统查找出同一执行日期内，同机号的出/进航班
        // let jointUrl = ''
        // postData(jointUrl, data).then(res => {
        //   if (res.data.code == 0) {
        //     this.statusData.jointData.tableData = res.data.data
        //   } else {
        //     this.showError('获取可拼接的航班', '请重新尝试 !')
        //   }
        // })
      }
      ...
      if (item.type == 'publish') {
        ...
        if (item.title == '撤销开始值机') {
          if (data.inOutFlag == 'A|D') { // 拼接返回出港的id
            this.$set(statusData.data, 'dynamicFlightIdD', data.dynamicFlightIdD)
          }
        }
        document.addEventListener('keypress', this.handleStatusConfirm, false)
        } else if (item.type == 'permission' || item.type == 'exchange') {
          statusData.visible = true
          document.addEventListener('keypress', this.handleStatusConfirm, false)
        } else if (item.type == 'jointFlt') {
          statusData.visible = true
          document.addEventListener('keypress', this.handleStatusConfirm, false)
        } else if (item.type == 'breakJointFlt') {
          this.revokeData.visible = true
          this.revokeData.info = `是否断开拼接？`
          document.addEventListener('keypress', this.handleStatusConfirm, false)
        }
    },
    handleStatusConfirm (event) {
      if ((event && event.keyCode == 13) || !event) {
        if (this.statusData.data.key == 'progressStatus' && this.statusData.data.value == 'ONR') { // 前起
          ...
          } else if (this.statusData.data.type == 'breakJointFlt') {
        ...
        } else if (this.statusData.data.counterFlag == true) {
          // '发布开始值机', '发布值机截止', '发布开始登机', '发布催促登机' 有值机区域和值机柜台
          this.$set(this.statusData.data, 'checkinRegion', this.statusData.data.checkinRegionArray.join())
          this.$set(this.statusData.data, 'checkinCounter', this.statusData.data.checkinCounterArray.join())
          this.postStatusSuccess()
        } else {
          this.postStatusSuccess()
        }
    ...
    closeStatusDialog () {
      ...
      this.statusData.dlyData.tableData = []
      this.statusData.dlyData.multipleSelection = []
      this.statusData.jointData.tableData = []
      this.statusData.jointData.singleSelection = {}
      // 关闭撤销弹框
    ...
    recoveryStandChange (data, item) {
      this.recoveryStandData.url = item.url
      if (data['gate']) {
        this.recoveryStandData.row = data
        this.recoveryStandData.id = data[this.formData.key]
        postData(this.recoveryStandData.url, this.recoveryStandData.id).then(res => {
          if (res.data.code == 0) {
            this.closeResourceDialog()
            this.showSuccess('该航班机位回收', '')
            if (res.data.data.flag) {
              // this.recoveryStandData
            }
            this.queryDataReq(1)
          } else {
            this.showError('该航班机位回收', '请重新尝试')
            this.recoveryStandData.loading = false
          }
        }).catch(() => {
          this.recoveryStandData.loading = false
        })
      ...
      /* this.$set(this.recoveryStandData.data, item.key, item.value)
      if (data.inOutFlag == item.inOutFlag) {
        this.$set(this.resourceData.data, 'dynamicFlightId', data.dynamicFlightId)
      } else {
        this.$set(this.resourceData.data, 'dynamicFlightId', data['dynamicFlightId' + item.inOutFlag])
      }
      this.recoveryStandData.visible = true
      document.addEventListener('keypress', this.handleResourceConfirm, false) */
    },
    handleRecoveryStandConfirm (data, item) {
      this.recoveryStandData.loading = true
      postData(this.recoveryStandData.url, this.recoveryStandData.id).then(res => {
        if (res.data.code == 0) {
          this.closeResourceDialog()
          this.showSuccess('该航班机位回收', '')
    ...
    closeRecoveryStandDialog () {
      this.recoveryStandData.loading = false
      this.recoveryStandData.visible = false
      document.removeEventListener('keypress', this.handleResourceConfirm)
    },
    ...
    publishDLYBatch () {
      if (this.tableData.multipleSelection.length > 0) {
        this.statusData.visible = true
        this.statusData.title = '批量发布延误'
        this.statusData.dlyData.tableData = this.tableData.multipleSelection
        queryAll('/basicdata/abnormalreason/queryAll', {abnormalFlag: 'Y'}).then(res => {
          if (res.data.code == 0) {
            this.statusData.options.reasonList = res.data.data
            this.$set(this.statusData.data, 'intAbnormalResson', null)
            this.$set(this.statusData.data, 'abnormalResson', null)
          } else {
            this.showError('获取异常状态列表', '请重新尝试 !')
          }
        })
    ...
    handleDlyBatchConfirm () {
      ...
      postData(this.statusData.url, data).then(res => {
        if (res.data.code == 0) {
          ...
          this.statusData.loading = false
          this.showError('批量发布延误', '请重新尝试 !')
        }
      })
    },
    handleDlySelectionChange (val) {
      this.statusData.dlyData.multipleSelection = val
    },
    handleJointSelectionChange (row) {
      this.statusData.jointData.tableData.forEach(item => {
        // 排他,每次选择时把其他选项都清除
        if (item.dynamicFlightId !== row.dynamicFlightId) {
          item.checked = false
        }
      })
      console.log(row)
      this.statusData.jointData.singleSelection = row
    },
    handleJointConfirm () {
      // todo: 发送后台的数据格式和url
      this.$set(this.statusData.data, 'jointFltData', this.statusData.jointData.singleSelection)
      this.statusData.loading = true
      postData(this.statusData.url, this.statusData.data).then(res => {
        if (res.data.code == 0) {
          this.showSuccess('操作')
          this.closeStatusDialog()
          this.$delete(this.statusData.data, 'jointFltData')
          this.queryDataReq(1)
        } else {
          this.statusData.loading = false
          this.showError('航班拼接', '请重新尝试 !')
...
.change-aircraft-dialog>.el-dialog {
...
.menu-status-dialog .el-table__row--striped>td:first-child {
...
/* .menu-status-dialog .el-table__row--striped>.el-table_1_column_1 {
  background-color: #081e2d !important;
} */
</style>
