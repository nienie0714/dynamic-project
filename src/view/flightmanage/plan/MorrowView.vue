<template>
  <el-container direction="vertical">
      ...
      <div class="left-button">
        ...
        <el-dropdown trigger="click">
          ...
          <el-dropdown-menu slot="dropdown" class="morrow-button-dpd">
            <el-dropdown-item :divided="true" @click.native="changeJointReq">航班拼接</el-dropdown-item>
            <el-dropdown-item :divided="true" @click.native="changeUnJointReq">断开拼接</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        <el-dropdown trigger="click">
    ...
    <el-main class="whole-table-view mult-title-table">
      ...
      @handleDetail="handleDetail" @handleEdit="handleEdit" @handleDelete="handleDelete"
      @initEditable="initEditable" @saveEditable="saveEditable" @tableRowContextmenu="tableRowContextmenu">
        <template slot="slot-edit-div" slot-scope="scopeData">
          <el-select v-if="scopeData.field.type == 'select'" v-model="scopeData.item[scopeData.field.prop]" :placeholder="'请选择' + scopeData.field.label"
          :filterable="true" :clearable="true">
            <el-option v-for="option in scopeData.field.options" :key="option[scopeData.field.itemKey]" :value="option[scopeData.field.itemKey]" :label="option[scopeData.field.itemLabel]">
              <span style="float: left">{{ option[scopeData.field.itemLabel] }}</span>
            </el-option>
          </el-select>
          <el-radio-group v-else-if="scopeData.field.type == 'tabs'" v-model="scopeData.item[scopeData.field.prop]">
...
export default {
    ...
    return {
      ...
      formData: {
        ...
        formData: [
          ...
          {key: 'aircraftNo', label: '飞机号', type: 'select', filterable: true, optionsKey: 'Aircraft', itemKey: 'aircraftNo', itemLabel: 'aircraftNo', change: this.changeArcft},
          ...
          {key: 'stand', label: '机位', type: 'select', filterable: true, optionsKey: 'AircraftStand', itemKey: 'standNo', itemLabel: 'standNo', change: this.changeStand},
          ...
          // {key: 'confirmedFlag', label: '确认标识', type: 'tabs', tabsKey: 'isYOrN', options: []},
          ...
          {key: 'transferFlag', label: '结转标识', type: 'tabs', tabsKey: 'isYOrN', options: []},
          {
            ...
            tabelFields: [
              {prop: 'station', label: '经停站', propObj: 'stationObj', propCn: 'stationCn', type: 'select', width: '135', valueKey: 'airportIata', itemKey: 'airportIata', itemLabel: 'briefC'},
              ...
              {prop: 'sortkey', label: '航站顺序', width: '65', type: 'number', min: 0, max: 999}
            ],
            tempData: {},
            tempArr: []
          },
          {
            ...
            tabelFields: [
              ...
              {prop: 'flightNo', label: '航班号', width: '210', type: 'input', maxlength: 5, required: true}
            ],
            tempData: {},
            tempArr: []
          }
      ...
      tableData: {
        ...
        leftFields: [
          ...
          {prop: 'inOutFlagCn', label: '进出标识', hidden: false, width: 70}, /* , optionKey: 'fltTypeAll' */
          {prop: 'aircraftNo', label: '飞机号', width: 90, type: 'select', filterable: true, getOptions: '/basicdata/aircraft/queryAll', itemKey: 'aircraftNo', itemLabel: 'aircraftNo', edit: true},
          /* {prop: 'aircraftType', label: '机型', hidden: false, width: 50}, */
          {prop: 'stand', label: '机位', hidden: false, width: 50},
          {prop: 'terminal', label: '航站楼', hidden: false, width: 50},
          ...
          {prop: 'strExecDate', label: '执行日期', hidden: false, width: 120, formatter: this.formatterDay}
        ],
        centerFields: [
          {
            label: '始发站',
            ...
            fields: [
              ...
              {prop: 'std', label: '起飞', fixed: false, hidden: false, width: 140, formatter: this.formatterMin}
            ]
          },
          {
            label: '目的站',
            ...
            fields: [
              ...
              {prop: 'sta', label: '降落', fixed: false, hidden: false, width: 140, formatter: this.formatterMin}
            ]
          },
          {
            label: '经停站1',
            prop: 'lstPlanFlightStation',
            index: 0,
            hidden: false,
            width: 370,
            fields: [
        ...
        rightFields: [
          ...
          {prop: 'confirmedFlag', label: '确认标识', hidden: false, optionKey: 'isYOrN', width: 70}
        ],
        editData: {
  ...
  methods: {
      ...
      if (value.hasOwnProperty('std') && value.hasOwnProperty('sta')) {
        if (value.std && value.sta) {
          if (value.std >= value.sta) {
            ...
            callback(new Error(same.regInfo ? same.regInfo : '计划起飞时间必须计划降落结束时间'))
          }
        }
      }
      callback()
    ...
    changeJointReq () {
      this.queryUrl = '/planFlight/queryJointPlanFlights'
      this.queryDataReq()
    },
    changeUnJointReq () {
      this.queryUrl = '/planFlight/queryPlanFlightVOs'
      this.queryDataReq()
    },
    jointReq () {
    ...
    tableRowContextmenu (data) {
      ...
      if (this.menuData.data.aircraftNo) {
        this.menuData.items[0].disabled = false
      } else {
        this.menuData.items[0].disabled = true
      }
    ...
    dblClick (index) {
      if (this.tableData.data[index].confirmedFlag == 'Y') {
        this.changeData.visible = true
        this.changeData.method = this.dblClickConf
        this.changeData.index = index
        this.confirmData.id = this.tableData.data[index].strPlanFlightId
        document.addEventListener('keypress', this.handleChange, false)
      } else {
        this.confirmData.id = null
        this.dblClickConf(index)
      }
    },
    dblClickConf (index) {
      if (index != null) {
        this.initEditable(this.tableData.editData.index)
        this.tableData.editData.index = index
        this.tableData.editData.tempObj = JSON.parse(JSON.stringify(this.tableData.data[index]))
      } else {
        let index = this.changeData.index
        this.initEditable(this.tableData.editData.index)
        this.tableData.editData.index = index
        this.tableData.editData.tempObj = JSON.parse(JSON.stringify(this.tableData.data[index]))
        this.closeChangeDialog()
    ...
    changeEditBefore (index) {
      if (this.tableData.data[index].aircraftNo) {
        return true
      } else {
        return false
      }
    ...
    saveEditable (index) {
      let result = this.changeEditBefore(index)
      if (result) {
        let arr = this.tableData.data[index].vipFlag.split('|')
        let obj = {
          strPlanFlightId: this.tableData.data[index].strPlanFlightId,
          aircraftNo: this.tableData.data[index].aircraftNo,
          vipFlag: arr.length > 1 ? null : this.tableData.data[index].vipFlag
        }
        postData(this.tableData.editData.url, obj).then(res => {
          if (res.data.code == 0) {
            ...
            this.showError('设置', '请重新尝试')
          }
        })
      } else {
        this.showError('设置', '飞机号不能为空 !')
      }
    },
    initEditable (index) {
      if (index != null) {
        this.tableData.data[index] = JSON.parse(JSON.stringify(this.tableData.editData.tempObj))
      }
