  ...
  <el-container direction="vertical" :style="wholeBodyStyle">
    <el-header class="query-contain" style="height: 70px;">
      ...
      <div class="left-button">
        ...
        <el-button v-if="preData.preAuth == 1" type="success" @click="openPreDia()">预分配</el-button>
        ...
        <el-button type="success" @click="queryAllData(null, 1)">刷新</el-button>
      </div>
    </el-header>
                      ...
                      :style="(idx >= rightAutoNum) ? (!taskField.hidden && {width: 'calc(' + taskField.width + 'px - 10px'}) : (!taskField.hidden && {width: taskField.width + 'px'})">
                        ...
                        <td v-else-if="!taskField.hidden" :class="taskField.class" :width="taskField.width - 10">
                          <el-popover placement="bottom" width="160" trigger="click" class="popover-bottom"><!--  :ref="`popover-${index}-`" -->
                            ...
                            <div slot="reference">
                              <div v-if="item.taskDataMap[taskField.prop]" :class="item.taskDataMap[taskField.prop].taskDataCss + ' task-field-div'" @click="handleShow(index, taskField.prop)">
    ...
    <div class="dialog-form-edit task-pre-dialog" v-if="preData.visible">
      <el-dialog v-if="preData.visible" :visible.sync="preData.visible" :close-on-click-modal="false" width="740px" @close="handlePreClose">
        <div slot="title" class="dialog-header">
          <img :src="require('@img/title_deco.png')"/>
          <span class="header-title">预分配</span>
        </div>
        <div class="dialog-body">
            ...
            <div class="dialog-twrapper-tl">
              <div v-for="(item, index) in preData.preStatic" :key="index">
                <div class="twrapper-tl-item" :class="item.type == 1  ? '' : (item.type == 2 ? 'disabled' : 'actived')" @click="chosePreItem(item)">
                  ...
                  <div class="tl-item-font">
                    <div>{{`${item.index+1}.选择${item.title}`}}</div>
                    <div>{{item.cn}}</div>
                  </div>
                </div>
                <img v-if="item.index !== 3" :src="require('@img/icon_step_arrow.png')">
              </div>
            </div>
            ...
            <div class="dialog-twrapper-tr" v-if="preData.step !== 0">
              <div class="twrapper-tr-item">
                ...
                <div class="el-timepicker" v-if="preData.step == 3">
                  <!-- <el-time-picker ref="timePicker" is-range :clearable="false" maxlength v-model="preData.time" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间" placeholder="选择时间范围"
                  format='HH:mm' valueFormat='HH:mm:ss' @change="getPreFlight"></el-time-picker> -->
                  <el-input type="time" v-model="preData.time[0]" required @blur="changePreTime(0)">
                    <i class="el-icon-refresh el-input__icon" slot="suffix" @click="handleIconClick(0)"></i>
                  </el-input> ~ <el-input type="time" v-model="preData.time[1]" required @blur="changePreTime(1)">
                    <i class="el-icon-refresh el-input__icon" slot="suffix" @click="handleIconClick(1)"></i>
                  </el-input>
                </div>
                <div>
          ...
          <div class="el-dialog-bwrapper">
            <el-main class="page-table-view">
              ...
              <el-row class="pre-info-normal">
                ...
                <el-col :span="4"><div class="pre-info-title">保障单元/人员</div></el-col>
                <el-col :span="6"><div class="pre-info-title">航班号</div></el-col>
                <el-col :span="2"><div class="pre-info-title">机位</div></el-col>
                <el-col :span="2"><div class="pre-info-title">到达</div></el-col>
                <el-col :span="2"><div class="pre-info-title">起飞</div></el-col>
                <el-col :span="3"><div class="pre-info-title pre-title-center">操作</div></el-col>
              </el-row>
              <div class="pre-info-contbox">
                <div>
                  <el-row class="pre-info-cont" v-for="(item, index) in preData.preArr" :key="index" :class="index % 2 == 0 ? 'row-single' : ''">
                    <el-col :span="4" style="margin-left: 5px">
                      <el-tooltip :content="item.TASK_CN" placement="top-start">
                          <span>{{item.TASK_CN}}</span>
                      </el-tooltip>
                    </el-col>
                    <el-col :span="4">
                    ...
                    <el-col :span="6">{{item.FLIGHT_NO}}</el-col>
                    <el-col :span="2">{{item.STAND}}</el-col>
                    <el-col :span="2">{{item.TA == null ? '--:--' : item.TA.substr(11)}}</el-col>
                    <el-col :span="2">{{item.TD == null ? '--:--' : item.TD.substr(11)}}</el-col>
                    <el-col :span="3">
...
export default {
  ...
  data () {
    return {
     ...
     preButtonData: {
        ...
        width: '660px',
        class: ' dialog-cancel-warn',
        info: '修改确认信息可能会清空后置其他信息，确认执行此操作？',
        type: 0,
        data: {},
        style: {
          'max-height': window.innerHeight - 590 + 'px'
  ...
  methods: {
    ...
    handleIconClick (index) {
      if (index == 0) {
        this.$set(this.preData.time, 0, '00:00')
      } else {
        this.$set(this.preData.time, 1, '23:59')
      }
      this.getPreFlight()
    },
    skipToResGantt () {
    ...
    // 预分配权限
    preAuthority () {
      postData(this.preData.preAuthUrl, null).then(res => {
        if (res.data.code == 0) {
          this.preData.preAuth = res.data.data
        }
      })
    },
    // 预分配
    ...
    getPreFlight () {
      ...
      postData(this.preData.flightUrl, obj).then(res => {
        if (res.data.code == 0) {
          that.preData.tempObj = null
          let objArr = _.filter(that.preData.preArr, (o) => { return o.TASK_NO == that.preData.data.TASK.TASK_NO })
          if (objArr.length == 0) {
    ...
    chosePreItem (data) {
      ...
      if (data.type == 0) {
        this.preButtonData.visible = true
      }
    },
    handleConfirmDia () {
      this.preButtonData.visible = false
    ...
    handlePreEdit (data, index) {
      ...
      this.getPreFlight()
      this.preData.tempArr.push(Object.assign({}, {FLIGHT_NO}, {DYNAMIC_TASK_ID}, {TD}, {TA}, {STAND}))
    ...
    handleClearAll () {
      ...
      this.preData.data.TEAM_EMPS = []
      this.preData.data.STAND = []
      this.preData.data.FLIGHTS = []
      this.preData.time = ['00:00', '23:59']
    },
    // 提交
    handlePreCommit () {
...
.dialog-twrapper-tl>div {
  width: 100%;
  display: flex;
  flex-direction: column;
  justify-content: space-between;
}
.dialog-twrapper-tl>div>img {
  width: 20px;
  height: 10px;
  margin: 15px auto;
}
...
.twrapper-tl-item, .twrapper-tr-item {
  box-sizing: border-box;
  height: 90px;
  border-radius: 10px;
  padding-left: 20px;
...
/* 当前项*/
.twrapper-tl-item{
  background-color: rgba(54, 140, 180, 0.4) !important;
  border: 1px solid rgba(60, 166, 200, 0.7);
  box-shadow: 0px 0px 30px rgba(60, 166, 200, 0.4) inset, 0 0 20px rgba(60, 166, 200, 0.2) !important;
  -webkit-box-shadow: 0px 0px 30px rgba(60, 166, 200, 0.4) inset, 0 0 20px rgba(60, 166, 200, 0.2) !important;
  display: flex;
  align-items: center;
  box-sizing: border-box;
...
/* 已添项*/
.twrapper-tl-item.actived{
  background-color: rgba(54, 140, 180, 0.2) !important;
  border: none;
  box-shadow: none;
}
/* 未添项*/
.twrapper-tl-item.disabled{
  opacity: 0.4;
  border: none;
  box-shadow: none;
  cursor: not-allowed;
...
.tl-item-font>div:first-of-type:not(:last-of-type) {
  margin-bottom: 12px;
  color: #7a939e;
  font-size: 16px;
  height: 16px;
...
.twrapper-tr-item {
  ...
  white-space:nowrap;
  text-overflow:ellipsis;
  background-color: rgba(10, 35, 50, 0.4) !important;
  border: 1px solid rgba(60, 166, 200, 0.6) !important;
  box-shadow: 0 0 12px rgba(60, 166, 200, 0.4) inset, 0 0 12px rgba(60, 166, 200, 0.2) !important;
  border-radius: 10px;
}
.twrapper-tr-item>.tr-item-ul {
  margin: 0 6px;
  display: flex;
  flex-direction: column;
  color: #fff;
...
.page-table-view>.pre-info-cont:nth-child(1){
  background:rgba(60,166,200,0.4) !important;
}
.page-table-view>.pre-info-cont:nth-child(2){
  background:rgba(60,166,200,0.9) !important;
}
/* .single-row {
  background:rgba(60,166,200,0.4) !important;
}
...
.twrapper-tr-item>div:last-of-type {
  overflow-x: hidden;
  margin: 5px 0;
...
.task-pre-dialog .tr-item-list>div:first-of-type {
  width: 20px;
  height: 20px;
  line-height: 20px;
  margin-right: 12px;
}
.task-pre-dialog .tr-item-list>div:last-of-type {
  font-size: 16px;
  color: #fff;
}
