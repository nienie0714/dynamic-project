  ...
  <div class="msg-button-div">
    ...
    <div class="msg-button-right">
      <div :class="talk.show ? 'is-active': ''" @click="changeMsgType(talk.type)">{{talk.title}}
        <div v-if="talk.talkData.arr.length > 0 && require('lodash').filter(talk.talkData.arr, ['unread', true]).length > 0" class="unread-red-count">{{require('lodash').filter(talk.talkData.arr, ['unread', true]).length}}</div>
      </div>
    </div>
  </div>
  ...
  <el-dialog v-if="msgData.notice.sendMsg.visible" :visible.sync="msgData.notice.sendMsg.visible" :close-on-click-modal="false" width="860px" append-to-body class="other-dialog notice-dialog" @close="closeNoticeDialog">
    ...
    <div class="dialog-body">
      ...
      <div class="notice-cont">
        ...
        <el-select v-model="msgData.notice.sendMsg.cont" placeholder="请选择通告内容" :filterable="true" :clearable="true" @change="changeContent">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" :loading="msgData.notice.sendMsg.loading" :disabled="!this.msgData.notice.sendMsg.topic || !this.msgData.notice.sendMsg.content" @click="sendTopic">确 定</el-button>
    </div>
  </el-dialog>
...
export default {
  ...
  methods: {
      ...
      if (obj) {
        if (this.msgType.type == obj.key) {
          ...
          switch (obj.key) {
            ...
            temp = {
              airline: msg.flight_no.substr(0, 2),
              flightNo: msg.flight_no,
              time: `${this.latestDate.replace(/\//g, '-')} ${this.latestTime}`,
              msgCont: remark,
        ...
        if (res.data.code == 0) {
          ...
          _.forEach(res.data.data['rows'], (item) => {
            ...
            switch (this.msgType.type) {
              ...
              temp = {
                airline: obj.flight_no.substr(0, 2),
                flightNo: obj.flight_no,
                time: item.msgReceiveTime,
                msgCont: remark,
                msgSenderName: item.msgSenderName,
                taskName: obj.task_cn,
                msgConfirmTime: item.msgConfirmTime,
                msgUuid: item.msgUuid
              }
              break
    ...
    changeContent (value) {
      this.msgData.notice.sendMsg.content = value
    },
    skipTopic () {
    ...
    sendTopic () {
      if (!this.msgData.notice.sendMsg.topic) {
        this.showError('通告发布', '通告主题不能为空 !')
      } else if (!this.msgData.notice.sendMsg.content) {
        this.showError('通告发布', '通告内容不能为空 !')
      } else {
    ...
    closeNoticeDialog () {
      ...
      this.msgData.notice.sendMsg.cont = null
      ...
      if (!this.msgType.filterValue) {
        ...
        this.filterMsgData = _.filter(this.msgData[this.msgType.type].data, (item) => {
          return (_.some(item, (value, key) => {
            if (['msgTopic', 'airline', 'flightNo', 'msgCont', 'msgSenderName', 'taskName', 'empName'].includes(key)) {
              return value && ~value.indexOf(this.msgType.filterValue)
            } else {
              return false
            }
          }))
          // (item[key] && ~item[key].indexOf(this.msgType.filterValue)) || (item.msgCont && ~item.msgCont.indexOf(this.msgType.filterValue))
        })
