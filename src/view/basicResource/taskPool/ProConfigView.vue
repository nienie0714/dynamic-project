      ...
      <Table-view :permissions="permissions" :tableData="tableData" ref="basicTable" @handleDetail="handleDetail" @handleEdit="handleEdit" @handleDelete="handleDelete">
        <template slot="slot-scope" slot-scope="scope">
          <div class="pro-task-opr-div-row">
            <div v-for="(item, index) in scope.row[scope.field.prop]" :key="index">
              <el-popover placement="top" trigger="click">
                <div :name="`el-popover-${scope.row[tableData.key]}-${item.taskNo}`" class="pro-task-opr-pop">
                  <el-header>
                ...
                <div slot="reference">
                  <div :class="item.isActive=='Y'?'pro-task-div task-active':'pro-task-div task-dis-active'" @click="clickDiv(scope.row, index, scope.row[tableData.key], item.taskNo)">{{item.taskCn}}</div>
                </div>
              </el-popover>
            </div>
          </div>
            ...
            <div class="task-div-tasks-list">
              <div v-for="(item, index) in scope.data[scope.item.key]" :key="index" :class="(index % 2 == 0)?'task-pro-task task-single':'task-pro-task task-double'">
                ...
                <div class="task-pro-cont">
                  <el-popover v-for="(opr, idx) in item.operations" :key="idx" popper-class="task-pro-cont-pop" placement="top" trigger="click">
                    <el-row :id="`el-popover-${index}-${idx}`" :gutter="20" class="pro-task-opr-cont-pop">
                      <el-col :span="5">
                    ...
                    <div slot="reference" class="pro-task-opr-div" @click="setUlTaskDivId(index, idx)">
                          ...
                          <div v-if="scopeTemp.item.editIndex == index" class="tr-div-td">
                            <td v-for="field in scopeTemp.item.tabelFields" :key="field.prop" class="cont-tr-div" :width="field.width">
                              <div class="td-edit-div">
                                ...
                                <el-radio-group v-else-if="field.type == 'tabs'" :type="field.type" v-model.trim.lazy="item[field.prop]">
                                  <el-radio-button v-for="option in field.options" :key="option.key" :label="option.key"> {{ option.value }}</el-radio-button>
                                </el-radio-group>
...
export default {
  ...
  data () {
    return {
      showTableTaskDivId: null,
      showUlTaskDivId: null,
      // 基础路径
      ...
      tempProTaskDialog: {
        ...
        formData: [
          ...
          {key: 'taskCn', label: '任务名称', type: 'span'},
          {
            ...
            tabelFields: [
              {prop: 'operationCn', label: '操作名称', width: '105', type: 'span'},
      ...
      tableData: {
        ...
        multipleSelection: [],
        fields: [
          {prop: 'processCn', label: '流程名称', fixed: 'left', hidden: false, sortable: true, width: '300'},
  ...
  methods: {
    scrollTable () {
      var tableBody = document.getElementsByClassName('el-table__body-wrapper')[0]
      let that = this
      tableBody.onscroll = function () {
        if (that.showTableTaskDivId) {
          let div = document.getElementsByName(that.showTableTaskDivId)[2].parentElement
          div.style.display = 'none'
        }
      }
    },
    scrollUl () {
      var ulBody = document.getElementsByClassName('task-div-tasks-list')[1]
      let that = this
      ulBody.onscroll = function () {
        if (that.showUlTaskDivId) {
          let div = document.getElementById(that.showUlTaskDivId).parentElement
          div.style.display = 'none'
        }
      }
    },
    ...
    clickDiv (row, index, key, prop) {
      ...
      this.setTableTaskDivId(key, prop)
    },
    getCol (taskType) {
      switch (taskType) {
        case 'V':return '特车任务'
    ...
    editOpr (scopeTemp, index) {
      ...
      this.tempData = JSON.parse(JSON.stringify({
        ...
        reserved1: scopeTemp.data[scopeTemp.item.key][index].reserved1
      }))
    ...
    editTask (scope, index) {
      ...
      for (let i = 0; i < this.tempProTaskDialog.formData.length; i++) {
        if (this.tempProTaskDialog.formData[i].key == 'processCn') {
          ...
          for (let x = 0; x < this.tempProTaskDialog.formData[i].tabelFields.length; x++) {
            if (this.tempProTaskDialog.formData[i].tabelFields[x].hasOwnProperty('optionKey')) {
              this.$store.commit('setOption', this.tempProTaskDialog.formData[i].tabelFields[x].optionKey)
              let temp = this.$store.getters.getOption
              this.tempProTaskDialog.formData[i].tabelFields[x].options = temp
              break
            }
          }
    ...
    setTableTaskDivId (index, prop) {
      this.showTableTaskDivId = `el-popover-${index}-${prop}`
    },
    setUlTaskDivId (index, prop) {
      this.showUlTaskDivId = `el-popover-${index}-${prop}`
    }
  }
}
</script>
