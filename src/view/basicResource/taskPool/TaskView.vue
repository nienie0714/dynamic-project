                  ...
                  <tbody>
                    <tr v-for="(item, index) in scopeTemp.data[scopeTemp.item.key]" :key="index" :class="(index % 2 > 0)?'task-pro-cont-table-tr tr-single':'task-pro-cont-table-tr tr-double'">
                      ...
                      <div v-else class="tr-div-td">
                        <td v-for="field in scopeTemp.item.tabelFields" :key="field.prop" class="cont-tr-div" :width="field.width">
                          <div v-if="['reserved1'].includes(field.prop)">{{item[field.prop]=='0'?'无':(item[field.prop]=='1'?'绑定':'解绑')}}</div>
      ...
      formData: {
        ...
        formData: [
          ...
          {key: 'sortkey', label: '排序码', type: 'input'},
          {
            ...
            tabelFields: [
              {prop: 'operationNo', label: '操作编号', width: '130', type: 'input', maxlength: 20, required: true},
              {prop: 'operationCn', label: '操作名称', width: '160', type: 'input', maxlength: 20, required: true},
              {prop: 'reserved1', label: '绑定车辆', width: '120', type: 'tabs', maxlength: 20, required: true, optionKey: 'bindVehicle'},
              {prop: 'sortkey', label: '排序码', width: '72', type: 'number', min: 0, max: 999}
            ],
            tempData: {},
            deleteData: {
              visible: false,
  ...
  methods: {
    ...
    addOpr (scopeTemp) {
      ...
      scopeTemp.item.tabelFields.forEach(field => {
        if (field.prop == 'reserved1') {
          this.$set(obj, field.prop, '0')
        } else {
        ...
        if (res.data.code == 0) {
          for (let i = 0; i < this.formData.formData.length; i++) {
            if (this.formData.formData[i].type != 'slot') {
              ...
              this.$store.commit('setOption', this.formData.formData[i].tabelFields[2].optionKey)
              let temp = this.$store.getters.getOption
              this.$set(this.formData.formData[i].tabelFields[2], 'options', temp)
              this.$set(this.formData.formData[i], 'value', res.data.data)
              this.$set(this.formData.formData[i], 'isHidden', false)
              this.formData.formData[i].editIndex = null
            }
